...
VALUES (
'2025-09-15', 'Notebook Dell Inspiron',
'Informática', 3500.00, 1, 'João Silva', 'Rio de
Janeiro', 'RJ'
);
...
produtos VARCHAR(100), categoria VARCHAR(100), valor_unitário INT);

# Pedidos

CREATE TABLE pedidos (id INT PRIMARY KEY,
cliente_id INT, FOREIGN KEY (cliente_id) REFERENCES
clientes(id), produtos_id INT,
FOREIGN KEY (produtos_id) REFERENCES produtos(id)); 

INSERT INTO produtos (id, produtos, categoria, valor_unitário) VALUES
	(1, 'Mouse', 'Periféricos', 200),
    (2, 'Mesa', 'Movel', 300),
    (3, 'Cabo', 'Utensílios', 20),
    (4, 'Caneta', 'Utensílios', 5),
    (5, 'Monitor', 'Periféricos', 5000),
    (6, 'Anel', 'Jóia', 1000),
    (7, 'Corrente', 'Jóia', 700),
    (8, 'Tenis', 'Vestuário', 1200),
    (9, 'Calça', 'Vestuário', 100),
    (10, 'Celular', 'Comunicação', 2000);
    
    INSERT INTO pedidos (id, cliente_id, produtos_id) VALUES
		(1, 4, 3),
        (2, 2, 5),
        (3, 6, 10),
        (4, 3, 9),
...
# Tabela dimensão são cadastros de caracteriscas unicas (que n se repetem)

# Tabela de fatos são ações que se repetem 
